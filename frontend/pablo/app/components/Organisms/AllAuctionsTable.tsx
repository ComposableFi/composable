...
export const AllAuctionsTable: React.FC<TableContainerProps> = ({
  ...
  const { setActiveAuctionsPool, liquidityBootstrappingPools } = useStore();
  ...
  return (
    <TableContainer {...rest}>
      <Table sx={{ minWidth: 420 }} aria-label="autions table">
        ...
        <TableBody>
          {liquidityBootstrappingPools.list.slice(0, count).map((lbPool: LiquidityBootstrappingPool) => (
            <TableRow
              onClick={() => {
      ...
      {liquidityBootstrappingPools.list.length > count && (
        <Box
      ...
      {liquidityBootstrappingPools.list.length <= count && liquidityBootstrappingPools.list.length > limit && (
        <Box
