...
const createSwapsSlice: StoreSlice<SwapsSlice> = (set) => ({
  swaps: {
    ...
    setSelectedAsset: (id, side) => set((state) => {
      if (side === "base") {
    ...
    setSelectedPool: (pool) => set((state) => {
      state.swaps.selectedPool = pool;
      return state;
    }),
    setSpotPrice: (price) => set((state) => {
      state.swaps.spotPrice = price;
      return state;
    }),
    resetSwaps: () => set((state) => {
    ...
    setTokenAmounts: (amounts) => set((state) => {
