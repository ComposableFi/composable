...
export default function useLiquidityPools(liquidityFilter: boolean = false): {
  ...
  useAsyncEffect(async (): Promise<void> => {
    ...
    const { uniswapConstantProduct } =
      await LiquidityPoolFactory.fetchPermissionedPools(parachainApi, assets);
    ...
    for (const pool of uniswapConstantProduct) {
      const underlyingAssets = pool
        .getLiquidityProviderToken()
        .getUnderlyingAssets();
      const baseAsset = assets.find((x) =>
        (x.getPicassoAssetId(true) as BigNumber).eq(
          underlyingAssets[0].getPicassoAssetId(true) ?? new BigNumber(0)
        )
      );
      const quoteAsset = assets.find((x) =>
        (x.getPicassoAssetId(true) as BigNumber).eq(
          underlyingAssets[1].getPicassoAssetId(true) ?? new BigNumber(0)
        )
      );
      const pabloPools = await fetchPabloPools(
        (pool.getPoolId(true) as BigNumber).toNumber()
      );
      const volume =
        pabloPools.length > 0
          ? fromChainIdUnit(BigInt(pabloPools[0].totalVolume))
          : new BigNumber(0);
      ...
      if (baseAsset && quoteAsset) {
        ...
        if (liquidityFilter) {
          lpTokenBalance = await lpToken.balanceOf(
            (selectedAccount as InjectedAccountWithMeta).address
          );
        ...
        const stakingRewardPool =
          await PabloStakingRewardPool.fetchStakingRewardPool(
            parachainApi,
            lpToken.getPicassoAssetId(true) as BigNumber
          );

        const rewards = stakingRewardPool.getRewards();
        const rewardAssetIds = Array.from(
          stakingRewardPool.getRewards().keys()
        );
        const rewardAssets = assets.filter((rewardAsset) =>
          rewardAssetIds.includes(rewardAsset.getPicassoAssetId() as string)
        );

        const prices = await apollo.getPrice([
          baseAsset,
          quoteAsset,
          ...rewardAssets,
        ]);
        ...
        const baseAssetAmount = await pool.getAssetLiquidity(
          baseAsset.getPicassoAssetId(true) as BigNumber
        );
        const quoteAssetAmount = await pool.getAssetLiquidity(
          quoteAsset.getPicassoAssetId(true) as BigNumber
        );
        const totalValueLocked = baseAssetAmount
          .times(baseAssetValue)
          .plus(quoteAssetAmount.times(quoteAssetValue));
        ...
        for (const reward of rewardAssetIds) {
          ...
          if (rewardConfig) {
            const rewardAsset = rewardAssets.find(
              (asset) => (asset.getPicassoAssetId() as string) === reward
            );
            if (rewardAsset) {
              const rewardAssetIdStr =
                rewardAsset.getPicassoAssetId() as string;
              const rewardPerDay = rewardConfig.getRewardsPerDay();
              rewardsPerDay.push(
                ClaimableAsset.fromAsset(rewardAsset, rewardPerDay)
              );
              apy =
                apy +
                calculateStakingRewardsPoolApy(
                  prices[rewardAssetIdStr],
                  rewardPerDay,
                  lpTokenValue.times(lpTokensStaked)
                ).toNumber();
            }
          }
        }
