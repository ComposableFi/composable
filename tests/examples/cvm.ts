...
import { CwCvmRuntimeClient } from "cvm-cw-types/dist/cw-cvm-runtime/CwCvmRuntime.client"
...
const sender = await wallet.getAccounts().then(accounts => {
        if (accounts[0]) {
...
const client = new CwCvmRuntimeClient(rawClient, sender, "centauri1c676xpc64x9lxjfsvpn7ajw2agutthe75553ws45k3ld46vy8pts0w203g")
...
const result = await client.executeProgram(msg, "auto", undefined, [coin])
