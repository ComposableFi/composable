...
in pkgs.stdenv.mkDerivation {
    ...
    container_id=$(podman run --rm -d -u$(id -u):$(id -g) -p9944:9944 -p9988:9988 -p9998:9998 docker.io/composablefi/composable-sandbox:latest)
    podman ps
    wait_for_ws_server_to_be_ready 127.0.0.1 9944
    wait_for_ws_server_to_be_ready 127.0.0.1 9988

    ${subxt}/bin/subxt codegen --url ws://localhost:9944 | rustfmt --edition=2018 --emit=stdout > $out/rococo_scale.rs
    ${subxt}/bin/subxt codegen --url ws://localhost:9988 | rustfmt --edition=2018 --emit=stdout > $out/composable_scale.rs
