...
export async function createConsProdPoolforFnft(
  ...
  const {data: [collectionId, who, admin]} = events[0];
...
export async function stakeLpTokens(
  ...
  const {data: [currencyId, who, amount]} = await sendAndWaitForSuccess(
    api,
    lpProviderWallet,
...
export async function createStakingRewardPool(
  ...
  const shareAssetId = stakeAssetId + 100000;
...
export async function stakeMultipleLpTokens(
  api: ApiPromise,
  poolId: number,
  lpProvider: KeyringPair,
  stakeAmount: number,
  ...
  const txs = [];
  for (let i = 0; i < nbOfStakes; i++) {
    const poolIdP = api.createType("u128", poolId);
    const amountParam = api.createType("u128", Pica(stakeAmount));
    const duration = api.createType("u64", lockTime);
    txs.push(api.tx.stakingRewards.stake(poolIdP, amountParam, duration))
  }
...
export async function getUserFnfts(api: ApiPromise, stakerWallet: KeyringPair) {
  ...
  const instances = Array.from(instancesRaw.unwrap());
  return instances;
}
