...
import {
  ...
  dotOncomposable,
  ksmOncentauri,
  ksmOnComposable, osmosisAddress,
  picaIdOnComposable, picaOnOsmosis,
  picassoFeeAddress,
  usdtOnCentauri,
  usdtOnComposable
} from "./constants";
...
export function createIbcTransferExtrinsic(
  ...
  let params;
  if (toChannel === 'centauri' || toChannel === 'osmosis') {
...
export async function queryTotalIssuanceOfTokenOnCentauri(denom: string){
  const {stdout} = await exec(`~/go/bin/centaurid query bank total --denom ${denom} --output json`);
  const formattedBalance = JSON.parse(stdout);
  return formattedBalance.amount;
}

export async function queryTotalIssuanceOnOsmosis(denom: string) {
  const {stdout} = await exec(`~/go/bin/osmosisd query bank total --denom ${denom} --output json`);
  const formattedBalance = JSON.parse(stdout);
  return formattedBalance.amount;
}
...
export async function getStateOfFundsForPicassoToCentauri(api: ApiPromise, assets: number[], wallet: KeyringPair) {
  ...
  const preBalancesOnOsmosis = await queryBalancesOnOsmosis(osmosisAddress);
  return {
...
export function createIbcTxsForBatch(
  ...
  if (toChain === 'centauri') {
    ...
    channelId = 0;
  } else if (toChain == 'osmosis') {
    channelId = 0;
    memo = `{"forward":{"receiver":"${osmosisAddress}","port":"transfer","channel":"channel-3","timeout":600000000000,"retries":0}}`;
  } else {
    channelId = 1;
  }
  const txsArr = assets.map((asset, index) => {
    return createIbcTransferExtrinsic(api, receiver, asset, amounts[index], channelId, toChain, memo);
  })
  return txsArr;
}
...
export async function queryBalancesOnOsmosis(address: string){
  const {stdout} = await exec(`~/go/bin/osmosisd query bank balances ${address} --output json`);
  const formattedBalance = JSON.parse(stdout);
  const balances = parseBalancesFromOsmosis(formattedBalance.balances);
  return balances;
}

export async function parseBalancesFromOsmosis(balances: []){
  const retBalances = {
    "dot": '0',
    "usdt": '0',
    "ksm": '0',
  ...
  balances.map((balance: { denom: string, amount: string }) => {
    if (balance.denom === 'dotOnOsmosis') retBalances.dot = balance.amount;
    if (balance.denom === 'usdtOnOsmosis') retBalances.usdt = balance.amount;
    if (balance.denom === 'ksmOnOsmosis') retBalances.ksm = balance.amount;
    if (balance.denom === picaOnOsmosis) retBalances.pica = balance.amount;
  });
  return retBalances;
}

export async function queryBalanceOnOsmosis(address: string, denom: string) {
  const {stdout} = await exec(`~/go/bin/osmosisd query bank balances ${address} --denom ${denom} --output json`);
  const formattedBalance = JSON.parse(stdout);
  return formattedBalance.amount;
}

export async function queryBalanceOnCentauri(address: string, denom: string){
  const {stdout} = await exec(`~/go/bin/centaurid query bank balances ${address} --denom ${denom} --output json`);
  const formattedBalance = JSON.parse(stdout);
  return formattedBalance.amount;
}