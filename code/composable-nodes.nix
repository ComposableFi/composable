{ self, ... }:
{
  perSystem = { config, self', inputs', pkgs, system, systemLib, systemCommonRust, ... }:
    {
      # Add the npm-buildpackage overlay to the perSystem's pkgs
      packages = rec {
        composable-node = systemLib.crane-nightly.buildPackage (systemCommonRust.common-attrs // {
          name = "composable";
          cargoArtifacts = systemCommonRust.common-deps;
          cargoBuildCommand = "cargo build --release --package composable --features=builtin-wasm";
          ...
          COMPOSABLE_RUNTIME = "${self'.packages.composable-runtime}/lib/runtime.optimized.wasm";
          installPhase = ''
            mkdir -p $out/bin
        ...
        composable-node-release = systemLib.crane-nightly.buildPackage (systemCommonRust.common-attrs
          // {
          name = "composable";
          cargoArtifacts = systemCommonRust.common-deps;
        ...
        composable-bench-node = systemLib.crane-nightly.cargoBuild (systemCommonRust.common-bench-attrs
          // {
          name = "composable";
          cargoArtifacts = self'.packages.common-bench-deps;
          cargoBuildCommand = "cargo build --release --package composable";
          DALI_RUNTIME = "${self'.packages.dali-bench-runtime}/lib/runtime.optimized.wasm";
          PICASSO_RUNTIME =
