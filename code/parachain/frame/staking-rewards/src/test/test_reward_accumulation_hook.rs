...
fn test_reward_update_calculation() {
	new_test_ext().execute_with(|| {
		...
		// the expected unlocked balance for each block surpassed
		let expected = [
			(1, PICA::units(12)),
			(2, PICA::units(24)),
			(3, PICA::units(36)),
		...
		for (block_number, expected_total_rewards) in expected {
			...
			let available_rewards =
				<Test as crate::Config>::Assets::balance(PICA::ID, &pool_account);

			assert_eq!(
				available_rewards, expected_total_rewards,
...
pub(crate) fn check_rewards(expected: &[CheckRewards<'_>]) {
	...
	for CheckRewards { owner, pool_asset_id, pool_rewards } in expected.iter() {
		...
		for PoolRewards { reward_asset_id, expected_locked_balance, expected_unlocked_balance } in
			*pool_rewards
		{
