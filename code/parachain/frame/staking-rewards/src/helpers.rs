...
use crate::{
	claim_of_stake, validation::ValidSplitRatio, AccountIdOf, AssetIdOf, BalanceOf, Config, Event,
...
pub(crate) fn add_to_rewards_pot_and_assert<Runtime, RuntimeEvent>(
	who: AccountIdOf<Runtime>,
	pool_id: AssetIdOf<Runtime>,
		...
		OriginTrait<AccountId = <Runtime as frame_system::Config>::AccountId>,
{
	Runtime::assert_extrinsic_event(
		...
		Event::<Runtime>::RewardsPotIncreased { pool_id, asset_id, amount },
	)
}
...
// TODO(benluelo): Assert that the shares and fnft were burned & that the stake was transferred
// from the fnft asset account (fnft asset account should be empty)
		...
		OriginTrait<AccountId = <Runtime as frame_system::Config>::AccountId>,
{
	...
	if should_be_early_unstake {
		...
		if let Some(reward) = rewards_pool.rewards.get(&position_before_unstake.reward_pool_id) {
			...
			// individually like the rest of the reward assets since it "shares" a balance with
			// the staked asset (it's the same asset!)
...
"#,
			...
			assert_eq!(
				...
				treasury_rewards_balances_before_unstake
					...
					.copied()
					.unwrap_or_else(Runtime::Balance::zero)
					.add(position_before_unstake.stake.sub(expected_slashed_stake_amount)),
				r#"
treasury account's staked asset balance after an early unstake was not as expected.
staked asset id: {staked_asset:?}
...
"#,
		...
		// here, it's expected that the unstake was _not_ early and therefore should _not_ have
		// been slashed.
		assert_eq!(
			Runtime::Assets::balance(position_before_unstake.reward_pool_id, &owner),
...
"#,
	...
	for (reward_asset_id, reward) in &rewards_pool.rewards {
		...
		let unstake_reward_slashed_event = Event::UnstakeRewardSlashed {
...
"#,
		...
		// everything past this point is checked/ accounted for when checking the staked asset;
...
"#,
	...
	// reductions checks
	// allow redundant_clone here so that the stakes aren't modified, in case they're checked
	// after the following for loop in the future.
		...
		OriginTrait<AccountId = <Runtime as frame_system::Config>::AccountId>,
{
	match reward_config.clone() {
		RewardPoolConfiguration::RewardRateBasedIncentive {
			...
			Event::<Runtime>::RewardPoolCreated { pool_id: asset_id, owner, end_block },
