...
fn claim_for_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			Unlockings::<Test>::get(ALICE).unwrap(),
			vec![UnlockChunk { value: ksm(4.95f64), era: 4 },]
		);
		...
		assert_ok!(LiquidStaking::claim_for(RuntimeOrigin::signed(BOB), Id(ALICE)));
		assert_eq!(<Test as Config>::Assets::balance(KSM, &ALICE), ksm(90f64) + ksm(4.95f64));
...
fn test_on_initialize_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(LiquidStaking::staking_ledger(derivative_index).unwrap(), staking_ledger);
...
fn test_force_set_era_start_block_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(LiquidStaking::force_set_era_start_block(RuntimeOrigin::root(), 11));
...
fn test_force_set_current_era_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(LiquidStaking::force_set_current_era(RuntimeOrigin::root(), 12));
...
fn test_force_notification_received_work() {
	new_test_ext().execute_with(|| {
		let derivative_index = 0u16;
		let bond_amount = ksm(10f64);
		assert_ok!(LiquidStaking::stake(RuntimeOrigin::signed(ALICE), ksm(20f64),));
		...
		assert_eq!(
			...
			Some(XcmRequest::Bond { index: derivative_index, amount: bond_amount })
		);
		assert_noop!(
			LiquidStaking::notification_received(
				RuntimeOrigin::signed(ALICE),
...
fn test_storage_proof_approach_should_work() {
	...
	.unwrap();
	assert_eq!(result.into_iter().collect::<Vec<_>>(), vec![(key, Some(value))],);
}
...
fn test_verify_merkle_proof_work() {
	new_test_ext().execute_with(|| {
		...
		LiquidStaking::on_finalize(1);
		assert!(LiquidStaking::verify_merkle_proof(key, value, get_mock_proof_bytes()));
	})
}
...
fn reduce_reserves_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(LiquidStaking::stake(RuntimeOrigin::signed(ALICE), ksm(100f64)));
		assert_eq!(LiquidStaking::total_reserves(), ksm(0.5f64));
		// Reduce 20 KSM reserves
		assert_ok!(LiquidStaking::reduce_reserves(RuntimeOrigin::root(), Id(ALICE), ksm(0.2f64)));
...
fn cancel_unstake_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			...
			MatchingLedger {
				total_stake_amount: ReservableAmount { total: ksm(9.95f64), reserved: 0 },
				total_unstake_amount: ReservableAmount { total: 0, reserved: 0 }
			}
		);

		assert_ok!(LiquidStaking::cancel_unstake(RuntimeOrigin::signed(ALICE), ksm(6f64)));
		assert_eq!(
			MatchingPool::<Test>::get(),
			MatchingLedger {
				total_stake_amount: ReservableAmount { total: ksm(9.95f64), reserved: 0 },
...
fn fast_unstake_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Loans::collateral_asset(RuntimeOrigin::signed(BOB), KSM, true));
		assert_ok!(LiquidStaking::unstake(
			RuntimeOrigin::signed(ALICE),
			ksm(6f64),
		...
		assert_eq!(<Test as Config>::Assets::balance(KSM, &ALICE), 95952000000000u128);
