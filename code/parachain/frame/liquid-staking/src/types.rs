...
use sp_runtime::{traits::Zero, ArithmeticError, DispatchError, FixedPointOperand};
...
impl<Balance: BalanceT + FixedPointOperand> MatchingLedger<Balance> {
	...
	pub fn matching(
		...
		let (bond_amount, rebond_amount, unbond_amount) =
			if matches!(total_free_stake_amount.cmp(&total_free_unstake_amount), Less | Equal) {
				(Zero::zero(), Zero::zero(), total_free_unstake_amount - total_free_stake_amount)
			} else {
...
pub enum XcmRequest<T: Config> {
	Bond { index: DerivativeIndex, amount: BalanceOf<T> },
	BondExtra { index: DerivativeIndex, amount: BalanceOf<T> },
	Unbond { index: DerivativeIndex, amount: BalanceOf<T> },
	Rebond { index: DerivativeIndex, amount: BalanceOf<T> },
	WithdrawUnbonded { index: DerivativeIndex, num_slashing_spans: u32 },
	Nominate { index: DerivativeIndex, targets: Vec<T::AccountId> },
}
...
impl<AccountId, Balance: BalanceT + FixedPointOperand> StakingLedger<AccountId, Balance> {
	...
	pub fn new(stash: AccountId, value: Balance) -> Self {
		Self { stash, total: value, active: value, unlocking: vec![], claimed_rewards: vec![] }
	}
	...
	pub fn unbond(&mut self, value: Balance, target_era: EraIndex) {
		if let Some(mut chunk) = self.unlocking.last_mut().filter(|chunk| chunk.era == target_era) {
			...
			self.unlocking.push(UnlockChunk { value, era: target_era });
		};
