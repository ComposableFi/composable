...
impl BalanceToFixedPoint<FixedI128> for u128 {
	fn to_fixed(self) -> Option<FixedI128> {
		FixedI128::checked_from_integer(
			TryInto::<<FixedI128 as FixedPointNumber>::Inner>::try_into(self).ok()?,
		)
	}
}
...
impl TruncateFixedPointToInt for FixedI128 {
	fn truncate_to_inner(&self) -> Option<Self::Inner> {
		self.into_inner().checked_div(FixedI128::accuracy())
	}
}

impl TruncateFixedPointToInt for FixedU128 {
	fn truncate_to_inner(&self) -> Option<<Self as FixedPointNumber>::Inner> {
		self.into_inner().checked_div(FixedU128::accuracy())
	}
}
