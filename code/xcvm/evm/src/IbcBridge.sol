...
contract IBCBridge is Context, IIbcBridge, IModuleCallbacks {
    ...
    function _sendPacket(bytes memory data, uint64 timeout) internal virtual {
    ...
    function onRecvPacket(
        ...
        (
            ,
            // bytes memory interpreterOrigin not used
            IRouter.Origin memory origin,
            bytes memory program,
            bytes memory salt,
            address[] memory _assets,
            uint256[] memory _amounts
        ) = SDK.decodeIBCSpawn(packet.data, routerAddress);
