...
impl Command {
    pub async fn run(self, chain_endpoint: String) -> Result<(), Error> {
        match self.subcommands {
            Subcommands::Query(cosmwasm::Query {
                contract,
                gas,
                query,
            }) => {
                ...
                let params = rpc_params!(
                    contract.to_string(),
                    gas,
                    serde_json::to_vec(&query).map_err(|e| Error::Serialization(e.to_string()))?
                );
...
async fn rpc_call<Res: DeserializeOwned>(
    ...
    let text = client
        ...
        .body(serde_json::to_string(&request).map_err(|e| Error::Serialization(e.to_string()))?)
        .send()
        .await
    ...
    let result: Result<Res, Error> = response.result().map_err(|e| Error::Rpc(e.to_string()));
    result
}
