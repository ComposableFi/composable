...
an CVM-specific acknowledgement must be committed for the packet:
...
Queries register values of an `CVM` instance across chains. It sets the current `Result Register` to `QueryResult`. See section 3. on the semantics of registers and `RegisterValues`.
...
Encoding depends on native encodign of chain, abilitty serde shared encoding and chain performace vs price variation.
