	...
	fn try_from(spawn: Spawn) -> core::result::Result<Self, Self::Error> {
		...
		Ok(xc_core::Instruction::Spawn {
			...
			bridge_security: match spawn.security {
				0 => xc_core::BridgeSecurity::Insecure,
				1 => xc_core::BridgeSecurity::Optimistic,
				2 => xc_core::BridgeSecurity::Probabilistic,
				3 => xc_core::BridgeSecurity::Deterministic,
				_ => return Err(()),
			},
			assets: spawn
				.assets
	...
	TAssets: Into<Vec<(xc_core::AssetId, xc_core::Balance)>>,
{
	fn from(instruction: xc_core::Instruction<TNetwork, TAbiEncoded, TAccount, TAssets>) -> Self {
		match instruction {
			...
			xc_core::Instruction::Spawn { network, bridge_security, salt, assets, program } =>
			...
			xc_core::Instruction::Query { network, salt } =>
