...
. Allow compounding of staked position when the rewarded asset is the same as staked.
...
To compensate for this new adjustment, a reduction stem:[d_(n + 1)] of reward for each staker needs to be tracked,

stem:[x_(n+1) = P_(current) . s_(n+1) / (sum s_(n+1)) - d_(n+1)]
...
==== [Needs modification] RewardAccumulationHook
