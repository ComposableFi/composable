...
This section covers how the staking rewards are distributed using the https://github.com/ComposableFi/composable/tree/main/frame/staking-rewards[staking rewards pallet] and https://github.com/ComposableFi/composable/tree/main/frame/vesting[vesting pallet].

==== Creating LP/PICA/PBLO/Other Token Staking Reward Pools

In order to create the necessary reward pool as well as the rewarding rate for stakers the vested transfers model can be used. Following interface (trait function) plus an extrinsic that can execute it using the governance origin would be the solution.

[source,rust]
----
include::../frame/composable-traits/src/staking_rewards.rs[lines=205..228]

...
----

===== Processing Transfer of Vested Reward
