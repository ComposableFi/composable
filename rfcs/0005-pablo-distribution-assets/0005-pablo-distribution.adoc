...
* sub-accounts for the rewards pots,
** A prefix is needed to differentiate between the pallet "hot wallet" which stores the claimable funds and the "cold wallet" which transferred provided treasury funds.
... Hot wallet prefix: "Hot"
... Cold wallet prefix: "Cold"
* Lock the assets in the cold wallet so that funds can be transferred only when unlocked (defensive programming)
* Reward accumulation logic would just transfer funds from the cold wallet to the hot wallet according to the reward rate.
* In order to add funds to the cold wallet an extrinsic is needed as follows
** Input: rewardPoolId, AssetId, Balance
+
For governance proposals one can query storage to get the reward pool ID and create a prposal to call the above extrinsic.
...
while (assetId = stake_pools.has_next) is (yes)
    ...
    while (reward = rewards_pool.rewards.has_next) is (yes)
        :assert max_reward not exceeded;
        :derive pallet cold wallet for the pool;
        :new_reward = elapsed_time * reward.reward_rate;
        :unlock new_reward amount in cold wallet;
        :transfer new_reward to hot wallet;
