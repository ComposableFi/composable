...
❓ **Q1:** What are Composable Cosmos, Composable parachain and Picasso?
...
1. **Composable Cosmos Chain**: This is a native chain within the Cosmos ecosystem, specifically designed for interoperability. It connects to our Picasso parachain in the Kusama network via IBC (Inter-Blockchain Communication), facilitating seamless asset transfers and cross-chain functionalities.
2. **Composable Parachain**: Residing in the Polkadot ecosystem, the Composable Parachain is built for cross-chain functionality and also utilizes IBC to connect with our Picasso parachain in the Kusama network.
3. **Picasso:** An infrastructure layer on Kusama aiming for DeFi interoperability. Its native token is $PICA, and it connects to Composable Cosmos and Parachain via IBC.
...
- **Composable Parachain is connected to Polkadot Relay Chain:** via XCM (Cross-Consensus Messaging).
- **Picasso is connected to Kusama Relay Chain:** also via XCM.

Each of these chains has its own relay mechanisms and further connections. For example, the Composable Cosmos Chain is connected to Cosmos Hub and Osmosis via IBC.
...
![composable.png](images-faq/composable.png)
...
**Use Cases:** As of now, PICA, the native token of Picasso, can be used for participating in governance decisions and staking, but these functionalities are available only on the Composable Cosmos Chain.

**Tokenomics:** You can find more details on the **[Picasso Tokenomics Page](https://docs.composable.finance/networks/picasso/tokenomics)**.
...
📝 **A5:** Trustless transfers refer to the automated and secure process of moving assets across different blockchains without requiring an intermediary or third party. This is achieved through the use of smart contracts, thereby reducing the risk of human error or malicious activities.
...
This approach stands in contrast to some other transfer protocols that may require a layer of trust or manual validation, potentially introducing centralization risks.
