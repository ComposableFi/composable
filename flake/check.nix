    ...
    apps = {
        ...
        program = pkgs.writeShellApplication {
          ...
          text = ''
            ...
            if [[ ''${ACTIONS_RUNNER_DEBUG-"false"} = "true" ]]; then
              NIX_DEBUG_COMMAND=' --print-build-logs --debug --show-trace --verbose'
            fi
            set -o pipefail -o errexit
            # shellcheck disable=SC2086
            NIXPKGS_ALLOW_BROKEN=1 nix flake check --keep-going --no-build --allow-import-from-derivation --no-update-lock-file --accept-flake-config --fallback --print-build-logs $NIX_DEBUG_COMMAND --impure --option sandbox relaxed 2>&1 | tee "nix.check.log"  || true
            set +o pipefail +o errexit
      ...
      run-in-docker = {
        ...
        program = pkgs.writeShellApplication {
          ...
          runtimeInputs = [  ];
          text = ''
          docker run --rm --volume /var/run/docker.sock:/var/run/docker.sock --volume nix:/nix -it nixos/nix bash -c "nix run composable#''${1-} --print-build-logs --extra-experimental-features nix-command --extra-experimental-features flakes --print-build-logs --option sandbox relaxed --show-trace --accept-flake-config" 
          '';
        };
      };
    };
