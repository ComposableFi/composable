...
check_runtime() {
  VERSIONS_FILE="$1"
  add_spec_version="$(git diff "${LATEST_TAG_NAME}" "${GITHUB_SHA}" -- "${VERSIONS_FILE}" |
    sed -n -r "s/^\+[[:space:]]+spec_version: +([0-9]+),$/\1/p")"
  sub_spec_version="$(git diff "${LATEST_TAG_NAME}" "${GITHUB_SHA}" -- "${VERSIONS_FILE}" |
    sed -n -r "s/^\-[[:space:]]+spec_version: +([0-9]+),$/\1/p")"
  if [ "${add_spec_version}" != "${sub_spec_version}" ]; then
    ...
    add_impl_version="$(git diff "${LATEST_TAG_NAME}" "${GITHUB_SHA}" -- "${VERSIONS_FILE}" |
      sed -n -r 's/^\+[[:space:]]+impl_version: +([0-9]+),$/\1/p')"
    sub_impl_version="$(git diff "${LATEST_TAG_NAME}" "${GITHUB_SHA}" -- "${VERSIONS_FILE}" |
      sed -n -r 's/^\-[[:space:]]+impl_version: +([0-9]+),$/\1/p')"
