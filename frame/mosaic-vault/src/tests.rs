...
use sp_runtime::{
	helpers_128bit::multiply_by_rational, traits::AccountIdConversion, FixedPointNumber, Percent,
	Perquintill,
};
...
//         assert_noop!(MosaicVault::set_min_fee(Origin::signed(ALICE), 600),
// Error::<Test>::MinFeeAboveMaxFee);         assert_noop!(MosaicVault::set_min_fee(Origin::
// signed(ALICE), 120), Error::<Test>::MinFeeAboveFeeFactor);         // assert_ok!(
// MosaicVault::set_min_fee(Origin::signed(ALICE), 10));         //
...
//         assert_noop!(MosaicVault::set_max_fee(Origin::signed(ALICE), 200),
// Error::<Test>::MaxFeeAboveFeeFactor);         assert_noop!(MosaicVault::set_max_fee(Origin::
// signed(ALICE), 10), Error::<Test>::MaxFeeBelowMinFee);         assert_ok!(MosaicVault::
// set_max_fee(Origin::signed(ALICE), 15));         assert_eq!(MosaicVault::max_fee(), 15);
//     })
// }
...
fn test_set_asset_max_transfer_size() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			MosaicVault::set_asset_max_transfer_size(Origin::signed(BOB), MockCurrencyId::A, 200),
			BadOrigin
		);
		assert_ok!(MosaicVault::set_asset_max_transfer_size(
			Origin::signed(ALICE),
			MockCurrencyId::A,
			200
		));
...
fn test_set_asset_min_transfer_size() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			MosaicVault::set_asset_min_transfer_size(Origin::signed(BOB), MockCurrencyId::A, 50),
			BadOrigin
		);
		assert_ok!(MosaicVault::set_asset_min_transfer_size(
			Origin::signed(ALICE),
			MockCurrencyId::A,
			50
		));
...
fn test_set_max_transfer_delay() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			MosaicVault::set_max_transfer_delay(Origin::signed(ALICE), 80),
			Error::<Test>::MaxTransferDelayBelowMinimum
		);
	})
}
...
fn test_set_min_transfer_delay() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			MosaicVault::set_min_transfer_delay(Origin::signed(ALICE), 700),
			Error::<Test>::MinTransferDelayAboveMaximum
		);
	})
}
...
fn test_set_thresh_hold() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			MosaicVault::set_thresh_hold(Origin::signed(ALICE), 100),
...
fn test_add_supported_token() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			MosaicVault::add_supported_token(
				Origin::signed(BOB),
				MockCurrencyId::A,
				MockCurrencyId::A,
				remote_network_id,
		...
		assert_noop!(
			...
			Error::<Test>::MaxAssetTransferSizeBelowMinimum
		);

		assert_ok!(MosaicVault::add_supported_token(
			Origin::signed(ALICE),
			MockCurrencyId::A,
			MockCurrencyId::A,
			remote_network_id,
...
fn test_deposit() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			...
			Error::<Test>::TransferNotPossible
		);
		...
		assert_noop!(
			...
			Error::<Test>::TransferDelayBelowMinimum
		);
		...
		assert_noop!(
			...
			Error::<Test>::TransferDelayAboveMaximum
		);
		...
		assert_noop!(
			...
			Error::<Test>::AmountAboveMaxAssetTransferSize
		);
		...
		assert_noop!(
			...
			Error::<Test>::AmountBelowMinAssetTransferSize
		);

		// assert_noop!(MosaicVault::deposit(Origin::signed(ALICE), 500, MockCurrencyId::A, BOB,
		// remote_network_id, transfer_delay), <pallet_vault::Pallet<Test>>::Error::<Test>); - how
		// to use error type from another palet - vault - VaultDoesNotExist

		assert_ok!(MosaicVault::deposit(
			Origin::signed(ALICE),
			500,
			MockCurrencyId::A,
			BOB,
...
fn test_withdraw() {
	ExtBuilder::default().build().execute_with(|| {
		...
		} = deposit_completed
		{
			...
			assert_ok!(MosaicVault::withdraw(
				Origin::signed(RELAYER_ACCOUNT),
				BOB,
				900,
				MockCurrencyId::A,
