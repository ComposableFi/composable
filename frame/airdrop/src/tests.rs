use crate::{Error, mocks::{ethereum_address, generate_accounts, AccountId, Balance, Balances, ClaimKey, Airdrop, EthKey, ExtBuilder, Moment, Origin, System, MockRuntime, Timestamp, PROOF_PREFIX, VESTING_STEP, Stake,AirdropId}, AccountIdOf};
use codec::Encode;
...
const DEFAULT_FUNDED_CLIAM: bool = false;
...
fn with_recipients<R>(count: u128, reward: Balance, funded_claim: bool, vesting_period: Moment, execute: impl FnOnce(&dyn Fn(Moment), Vec<(AccountId, ClaimKey)>) -> R) -> R {
    let accounts = generate_accounts(count as _);
    let recipients = accounts.iter().map(|(_, account)| (account.as_remote_public(), reward, funded_claim)).collect();
...
fn with_default_recipients<R>(execute: impl FnOnce(&dyn Fn(Moment), Vec<(AccountId, ClaimKey)>) -> R) -> R {
    with_recipients(DEFAULT_NB_OF_CONTRIBUTORS, DEFAULT_REWARD, DEFAULT_FUNDED_CLIAM, DEFAULT_VESTING_PERIOD, execute)
}
...
mod create_airdrop_tests {
    ...
    #[test]
    #[allow(clippy::disallowed_methods)] // Allow `farme_support::pallet_prelude::ValueQuery`
    ...
    fn create_invalid_airdrop_bttf() {
        ...
        ExtBuilder::default().build().execute_with(|| {
            ...
            assert_noop!(Airdrop::create_airdrop(creator, start, vesting_schedule), Error::<MockRuntime>::BackToTheFuture);
...
mod add_recipient_tests {
    ...
    fn add_recipient_invalid_creator() {
        ...
        let recipients = accounts.iter().map(|(_, account)| (account.as_remote_public(), DEFAULT_REWARD, DEFAULT_FUNDED_CLIAM)).collect();

        ExtBuilder::default().build().execute_with(|| {
            Balances::make_free_balance_be(&CREATOR, 10_000);
            assert_ok!(Airdrop::create_airdrop(creator, start, vesting_schedule));
            assert_noop!(Airdrop::add_recipient(other, 1, recipients), Error::<MockRuntime>::NotAirdropCreator);
        })
    }
    ...
    fn add_recipient_insufficient_funds() {
        ...
        ExtBuilder::default().build().execute_with(|| {
            ...
            assert_noop!(Airdrop::add_recipient(creator, 1, recipients), pallet_balances::Error::<MockRuntime>::InsufficientBalance);
        })
    }
    ...
    fn add_recipient_airdrop_does_not_exist() {
        ...
        ExtBuilder::default().build().execute_with(|| {
            Balances::make_free_balance_be(&CREATOR, 10_000);
            assert_noop!(Airdrop::add_recipient(creator, 1, recipients), Error::<MockRuntime>::AirdropDoesNotExist);
        })
    }
    ...
    fn add_recipient_valid() {
        with_default_recipients(|_, accounts| {
            ...
            for (_, remote_account) in accounts {
                let recipient_fund = Airdrop::recipient_funds(1, remote_account.as_remote_public()).unwrap();
                assert_eq!(recipient_fund.total, DEFAULT_REWARD);
                assert_eq!(recipient_fund.funded_claim, DEFAULT_FUNDED_CLIAM);
            }
        });
    }
}
...
mod remove_recipient_tests {
    ...
    fn remove_recipient_invalid_creator() {
        ...
        with_default_recipients(|_, accounts| {
            assert_noop!(Airdrop::remove_recipient(other, 1, accounts[0].1.as_remote_public()), Error::<MockRuntime>::NotAirdropCreator);
        })
    }
    ...
    fn remove_recipient_already_claimed() {
        ...
        with_default_recipients(|set_moment, accounts| {
            ...
            assert_ok!(Airdrop::claim(Origin::none(), 1, accounts[0].clone().0, accounts[0].clone().1.proof(accounts[0].clone().0)));
            assert_noop!(Airdrop::remove_recipient(creator, 1, accounts[0].1.as_remote_public()), Error::<MockRuntime>::RecipientAlreadyClaimed);
        })
    }
    ...
    fn remove_recipient_trigger_prune() {
        ...
        with_default_recipients(|_, accounts| {
            ...
            for (_, remote_account) in accounts {
                assert_ok!(Airdrop::remove_recipient(creator.clone(), 1, remote_account.as_remote_public()));
            }
    ...
    fn remove_recipient_valid_no_prune() {
        ...
        with_default_recipients(|_, accounts| {
            assert_ok!(Airdrop::remove_recipient(creator.clone(), 1, accounts[0].1.as_remote_public()));
