...
fn register_asset() {
	new_test_ext().execute_with(||{
		let location = <Runtime as crate::Config>::ForeignAssetId::decode(&mut &XcmAssetLocation::RELAY_NATIVE.encode()[..]).unwrap();
		let ed = 42_u64.into();
		...
		assert_ok!(AssetsRegistry::register_asset(Origin::root(), location.clone(), ed, Some(ratio), Some(decimals)));
		let local_asset_id = AssetsRegistry::from_foreign_asset(location.clone()).unwrap();
		assert_eq!(AssetsRegistry::from_local_asset(local_asset_id), Some(ForeignMetadata { decimals: Some(decimals), location: location.clone() }));

		assert_noop!(
			AssetsRegistry::register_asset(Origin::root(), location, ed, Some(ratio), Some(decimals)),
...
fn update_asset() {
	new_test_ext().execute_with(||{
		...
		assert_ok!(AssetsRegistry::update_asset(Origin::root(), local_asset_id, location.clone(), Some(new_ratio), Some(new_decimals)));
		assert_eq!(AssetsRegistry::from_local_asset(local_asset_id), Some(ForeignMetadata { decimals: Some(new_decimals), location }));
...
fn set_min_fee() {
	new_test_ext().execute_with(||{
		...
		assert_eq!(AssetsRegistry::minimal_amount(target_parachain_id, foreign_asset_id.clone()), None);

		assert_ok!(AssetsRegistry::set_min_fee(Origin::root(), target_parachain_id, foreign_asset_id.clone(), Some(balance)));

		assert_eq!(AssetsRegistry::minimal_amount(target_parachain_id, foreign_asset_id), Some(balance));
	})
}