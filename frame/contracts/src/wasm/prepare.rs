...
// TODO(hussein-aitlahcen): make all error strongly typed.
...
impl<'a, T: Config> ContractModule<'a, T> {
	...
	fn ensure_internal_memory_limit(&self) -> Result<(), &'static str> {
		if self.module.memory_section().map_or(false, |ms| ms.entries().len() != 1) {
			...
			match export_entries.iter().find(|e| e.field() == name) {
				Some(export) => {
					...
					let fn_idx = match fn_idx.checked_sub(fn_space_offset as u32) {
						Some(fn_idx) => fn_idx,
						None => {
							// Underflow here means fn_idx points to imported function which we
							// don't allow!
					...
					if signature != func_ty.params() {
						return Err("export has wrong signature")
					}
				},
				None => {
...
fn check_and_instrument<C: ImportSatisfyCheck, T: Config>(
	...
	let result = (|| {
		...
		contract_module.ensure_internal_memory_limit()?;
