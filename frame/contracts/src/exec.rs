...
pub trait Executable<T: Config>: Sized {
	...
	///
	/// When the specified function is `Instantiate` the executable is stored and its
...
enum FrameArgs<'a, T: Config, E> {
	...
	Instantiate {
		...
		nonce: u64,
		/// The executable whose `instantiate` function is run.
		executable: E,
	...
	E: Executable<T>,
{
	...
	fn new_frame<S: storage::meter::State>(
		...
		let (account_id, contract_info, executable, delegate_caller, entry_point, nonce) =
			match frame_args {
				...
				FrameArgs::Query { dest, cached_info, delegated_call } => {
					let contract = if let Some(contract) = cached_info {
						contract
					} else {
						<ContractInfoOf<T>>::get(&dest).ok_or(<Error<T>>::ContractNotFound)?
	...
	fn run_recurse(
		...
		let result = match entry_point {
			...
			ExportedFunction::Instantiate => executable
				...
				.and_then(|x| {
					x.into_result()
						.map(|x| {
							add_event(Left(Event::Instantiated {
		...
		let convert_addr = |addr: String| {
			T::ConvertAccount::convert(addr).map_err(|_| {
				...
				ExecError { error: Error::<T>::DecodingFailed.into(), origin: ErrorOrigin::Callee }
			})
		};
					...
					|_,
					 ...
					 -> Result<ExecReturnValue, ExecError> {
						...
						let mut raise_event = |event: Either<Event<T>, CosmwasmEvent>| {
							...
							if let Right(cosmwasm_event) = event {
						...
						let sub_call_cont = match (sub_message_result, reply_on.clone()) {
							...
							(Err(e), ReplyOn::Always | ReplyOn::Error) => {
								...
								SubCallCont::Reply(SubMsgResult::Err(format!("{:?}", e.error)))
							},
							// Rollback and abort
	...
	fn run(&mut self, executable: E, input_data: Vec<u8>) -> Result<ExecReturnValue, ExecError> {
		with_transaction(|| {
			...
			let r = self.run_recurse(executable, input_data, &mut raise_event);
			match r {
				Ok(x) => {
					let encoded_events = events
						.into_iter()
						.map(|event| -> Result<Either<Event<T>, Vec<u8>>, ExecError> {
							match event {
								Left(native_event) => Ok(Left(native_event)),
								Right(cosmwasm_event) => serde_json::to_vec(&cosmwasm_event)
									.map(Right)
									.map_err(|_| ExecError {
					...
					match encoded_events {
						Ok(events) => {
							events.into_iter().for_each(|event| match event {
								Left(native_event) => {
									deposit_event::<T>(vec![], native_event);
								},
								Right(cosmwasm_event) => {
									self.deposit_event(vec![], cosmwasm_event);
								},
							});
							TransactionOutcome::Commit(Ok(x))
						},
	...
	fn pop_frame(&mut self, persist: bool) {
		...
		if !persist && self.top_frame().entry_point == ExportedFunction::Instantiate {
...
mod tests {
	...
	fn input_data_to_instantiate() {
		let input_data_ch = MockLoader::insert(Instantiate, |ctx, _| {
			assert_eq!(ctx.input_data, &[1, 2, 3, 4]);
			exec_success()
		});
	...
	fn refuse_instantiate_with_value_below_existential_deposit() {
		let dummy_ch = MockLoader::insert(Instantiate, |_, _| exec_success());
	...
	fn instantiation_work_with_success_output() {
		let dummy_ch = MockLoader::insert(Instantiate, |_, _| {
	...
	fn instantiation_traps() {
		let dummy_ch = MockLoader::insert(Instantiate, |_, _| Err("It's a trap!".into()));
		let instantiator_ch = MockLoader::insert(Call, {
			let dummy_ch = dummy_ch.clone();
			move |ctx, _| {
				// Instantiate a contract and save it's address in `instantiated_contract_address`.
	...
	fn termination_from_instantiate_fails() {
		let terminate_ch = MockLoader::insert(Instantiate, |ctx, _| {
		...
		ExtBuilder::default().existential_deposit(15).build().execute_with(|| {
			...
			assert_eq!(
				...
				Err(Error::<Test>::TerminatedInInstantiate.into())
			);
	...
	fn recursive_call_during_constructor_fails() {
		let code = MockLoader::insert(Instantiate, |ctx, _| {
			assert_matches!(
	...
	#[test]
	fn nonce() {
		let fail_code = MockLoader::insert(Instantiate, |_, _| exec_trapped());
		let success_code = MockLoader::insert(Instantiate, |_, _| exec_success());
		let succ_fail_code = MockLoader::insert(Instantiate, move |ctx, _| {
			ctx.ext
				.instantiate(0, fail_code, ctx.ext.minimum_balance() * 100, vec![], &[])
				.ok();
			exec_success()
		});
		let succ_succ_code = MockLoader::insert(Instantiate, move |ctx, _| {
