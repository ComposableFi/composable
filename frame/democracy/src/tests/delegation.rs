...
fn single_proposal_should_work_with_delegation() {
	new_test_ext().execute_with(|| {
		...
		// Delegate first vote.
		assert_ok!(Democracy::delegate(Origin::signed(2), 1, DefaultAsset, Conviction::None, 20));
		let r = 0;
		assert_ok!(Democracy::vote(Origin::signed(1), r, aye(1)));
		assert_eq!(tally(r), Tally { ayes: 3, nays: 0, turnout: 30 });

		// Delegate a second vote.
		assert_ok!(Democracy::delegate(Origin::signed(3), 1, DefaultAsset, Conviction::None, 30));
		assert_eq!(tally(r), Tally { ayes: 6, nays: 0, turnout: 60 });

		// Reduce first vote.
		assert_ok!(Democracy::delegate(Origin::signed(2), 1, DefaultAsset, Conviction::None, 10));
		...
		assert_ok!(Democracy::delegate(Origin::signed(3), 2, DefaultAsset, Conviction::None, 30));
...
fn self_delegation_not_allowed() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Democracy::delegate(Origin::signed(1), 1, DefaultAsset, Conviction::None, 10),
			Error::<Test>::Nonsense,
		);
	});
}
...
fn cyclic_delegation_should_unwind() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Democracy::delegate(Origin::signed(2), 1, DefaultAsset, Conviction::None, 20));
		assert_ok!(Democracy::delegate(Origin::signed(3), 2, DefaultAsset, Conviction::None, 30));
		assert_ok!(Democracy::delegate(Origin::signed(1), 3, DefaultAsset, Conviction::None, 10));
		let r = 0;
		assert_ok!(Democracy::undelegate(Origin::signed(3), DefaultAsset));
		assert_ok!(Democracy::vote(Origin::signed(3), r, aye(3)));
		assert_ok!(Democracy::undelegate(Origin::signed(1), DefaultAsset));
...
fn single_proposal_should_work_with_undelegation() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Democracy::delegate(Origin::signed(2), 1, DefaultAsset, Conviction::None, 20));
		assert_ok!(Democracy::undelegate(Origin::signed(2), DefaultAsset));
