...
decl_storage! {
	trait Store for Module<T: Config> as Democracy {
		...
		pub ReferendumInfoOf get(fn referendum_info):
			...
			=> Option<ReferendumInfo<T::BlockNumber, T::Hash, BalanceOf<T>, AssetIdOf<T>>>;
...
impl<T: Config> Module<T> {
	...
	pub fn maturing_referenda_at(
		n: T::BlockNumber,
	) -> Vec<(ReferendumIndex, ReferendumStatus<T::BlockNumber, T::Hash, BalanceOf<T>, AssetIdOf<T>>)>
	{
	...
	fn ensure_ongoing(
		r: ReferendumInfo<T::BlockNumber, T::Hash, BalanceOf<T>, AssetIdOf<T>>,
	) -> Result<ReferendumStatus<T::BlockNumber, T::Hash, BalanceOf<T>, AssetIdOf<T>>, DispatchError>
	{
		match r {
	...
	fn bake_referendum(
		...
		status: ReferendumStatus<T::BlockNumber, T::Hash, BalanceOf<T>, AssetIdOf<T>>,
	) -> Result<bool, DispatchError> {
		let total_issuance = T::MultiCurrency::total_issuance(status.asset_id);
