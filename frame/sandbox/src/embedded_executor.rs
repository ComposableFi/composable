...
use wasmi::{
	...
	NotStartedModuleRef, RuntimeArgs, RuntimeValue, Signature, TableDescriptor, TableRef, Trap,
	TrapKind,
};
...
use crate::{Error, HostError, HostFuncType, MemoryError, ReturnValue, Value, TARGET};
...
impl Memory {
	pub fn new(memref: MemoryRef) -> Self {
		Memory { memref }
	}
}
...
impl super::SandboxMemory for Memory {
	...
	fn get(&self, ptr: u32, buf: &mut [u8]) -> Result<(), MemoryError> {
		self.memref.get_into(ptr, buf).map_err(|_| MemoryError::MemoryReadFailure)?;
		Ok(())
	}

	fn set(&self, ptr: u32, value: &[u8]) -> Result<(), MemoryError> {
		self.memref.set(ptr, value).map_err(|_| MemoryError::MemoryWriteFailure)?;
		Ok(())
	}
}
...
impl<T> super::SandboxInstance<T> for Instance<T> {
	...
	fn new(
		...
		let mut state = mk_state(&not_started_instance);
		let instance = {
		...
		Ok((Instance { instance, defined_host_functions, _marker: PhantomData::<T> }, state))
	}
