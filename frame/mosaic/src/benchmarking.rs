...
benchmarks! {
  ...
  }: _(RawOrigin::Signed(alice), network_id, asset_id, address, transfer_amount, false, alice.clone(), false)
  ...
  accept_transfer {
	...
	assert_ok!(Mosaic::<T>::update_asset_mapping(RawOrigin::Root.into(), asset_id, network_id.clone(), Some(remote_asset_id.clone())));
	  ...
	  let address = EthereumAddress([0u8; 20]);
		...
		assert_ok!(T::Assets::mint_into(asset_id, &alice, transfer_amount));
	  assert_ok!(Mosaic::<T>::transfer_to(RawOrigin::Signed(alice.clone()).into(), network_id.clone(), asset_id, address, transfer_amount, false, alice.clone(), false));
  ...
  claim_stale_to {
	...
	assert_ok!(Mosaic::<T>::update_asset_mapping(RawOrigin::Root.into(), asset_id, network_id.clone(), Some(remote_asset_id.clone())));
	  ...
	  let address = EthereumAddress([0u8; 20]);
		...
		assert_ok!(T::Assets::mint_into(asset_id, &alice, transfer_amount));
	  assert_ok!(Mosaic::<T>::transfer_to(RawOrigin::Signed(alice.clone()).into(), network_id, asset_id, address, transfer_amount, false, alice.clone(), false));
