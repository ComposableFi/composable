...
pub mod pallet {
		...
		<T as Config>::PoolSize: Ord
	{
		...
		fn create(
			...
			ensure!(Self::bounds_are_valid(&config.weight_bounds), Error::<T>::InvalidWeightBounds);
	...
	impl<T: Config> Pallet<T>
	{
		...
		fn do_spot_price(
			...
			let numerator: FixedBalance =
				asset_balance.checked_div(asset_weight).ok_or(ArithmeticError::Overflow)?;

			let denominator: FixedBalance = numeraire_balance
				.checked_div(numeraire_weight)
				.ok_or(ArithmeticError::Overflow)?;

			let result: FixedBalance =
				numerator.checked_div(denominator).ok_or(ArithmeticError::Overflow)?;
		...
		where <T as Config>::PoolSize: Ord {
	...
	impl<T: Config> Pallet<T>
	{
		...
		pub fn reserves_of(
			...
			for asset in assets {
				reserves
					.push(Reserve { asset_id: asset, amount: Self::balance_of(pool_id, &asset)? });
			}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn weights_are_normalized(
			...
			let sum: u128 = weights.iter().map(|weight| weight.weight.deconstruct().into()).sum();
		...
		pub fn weights_are_in_weight_bounds(
			...
			weights.iter().all(|weight| match (lower_bound, upper_bound) {
				(None, None) => true,
		...
		pub fn deposit_is_within_nonempty_pools_deposit_bounds(
			...
			let reserve_increase: FixedBalance =
				deposit.checked_div(reserve).ok_or(ArithmeticError::Overflow)?;
	...
	impl<T: Config> Pallet<T> {
		...
		fn increase_pool_asset_total_balance_storage(
			...
			PoolAssetTotalBalance::<T>::mutate(pool_id, asset_id, |balance| -> DispatchResult {
				*balance = balance.checked_add(amount).ok_or(ArithmeticError::Overflow)?;
				Ok(())
			})?;
		...
		fn increase_in_weighted_geometric_mean(
			...
			for deposit in deposits {
				...
				deposit_ratio =
					deposit_ratio.checked_add(asset_ratio).ok_or(ArithmeticError::Overflow)?;
			}
		...
		fn lps_share_of_pool(
			...
			for asset_id in assets.iter() {
				...
				lps_share.push(Reserve { asset_id: *asset_id, amount: lp_share_of_asset });
			}
