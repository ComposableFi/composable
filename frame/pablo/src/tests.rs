use crate::{
	mock::{Pablo, *},
	PoolConfiguration, PoolInitConfiguration,
};
...
#[test]
fn test_stable_swap_amp_zero_pool_creation_failure() {
...
//		let pool = StableSwap::pools(pool_id).expect("pool not found");
//		prop_assert_ok!(Tokens::mint_into(USDT, &BOB, value));
//		prop_assert_ok!(StableSwap::swap(Origin::signed(BOB), pool_id, CurrencyPair::new(USDC, USDT),
