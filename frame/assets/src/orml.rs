		...
		MultiCurrency<AccountId, Balance = T::Balance, CurrencyId = T::AssetId>,
{
	...
	fn total_balance(currency_id: Self::CurrencyId, who: &AccountId) -> Self::Balance {
		if currency_id == T::NativeAssetId::get() {
			return <<T as Config>::Currency>::total_balance(who)
		}
	...
	fn free_balance(currency_id: Self::CurrencyId, who: &AccountId) -> Self::Balance {
		if currency_id == T::NativeAssetId::get() {
			return <<T as Config>::Currency>::free_balance(who)
		}
	...
	fn ensure_can_withdraw(
		...
		if currency_id == T::NativeAssetId::get() {
			let new_balace = <<T as Config>::Currency>::free_balance(who)
				.checked_sub(&amount)
				.ok_or(DispatchError::Arithmetic(ArithmeticError::Underflow))?;
			return <<T as Config>::Currency>::ensure_can_withdraw(
				who,
				amount,
				WithdrawReasons::all(),
				new_balace,
	...
	fn transfer(
		...
		if currency_id == T::NativeAssetId::get() {
			return <<T as Config>::Currency>::transfer(
				from,
				to,
				amount,
	...
	fn withdraw(
		...
		if currency_id == T::NativeAssetId::get() {
			...
			// MultiCurrency trait.
			return <<T as Config>::Currency>::withdraw(
				who,
				amount,
				WithdrawReasons::all(),
				ExistenceRequirement::AllowDeath,
	...
	fn slash(
		...
		if currency_id == T::NativeAssetId::get() {
			// Drop the imbalance, causing the total issuance to decrease, in accordance with the
			// MultiCurrency trait.
			return <<T as Config>::Currency>::slash(who, amount).1
		}
	...
	<T as Config>::AssetId: MaybeSerializeDeserialize,
{
	...
	fn set_lock(
		...
		if currency_id == T::NativeAssetId::get() {
			<<T as Config>::Currency>::set_lock(lock_id, who, amount, WithdrawReasons::all());
			return Ok(())
		}
	...
	fn extend_lock(
		...
		if currency_id == T::NativeAssetId::get() {
			<<T as Config>::Currency>::extend_lock(lock_id, who, amount, WithdrawReasons::all());
			return Ok(())
		}
	...
	fn remove_lock(
		...
		if currency_id == T::NativeAssetId::get() {
			<<T as Config>::Currency>::remove_lock(lock_id, who);
			return Ok(())
		}
