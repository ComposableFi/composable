...
describe.only("tx.assets Tests", function() {
  ...
  describe("tx.assets.transfer Tests", function() {
    ...
    it("A wallet can `transfer` KSM to another wallet", async function() {
      ...
      const {data: [result]} = await sendAndWaitForSuccess(
        ...
        api.tx.assets.transfer(paraAsset, paraDest, paraAsset, paraKeepAlive)
      );
  ...
  /**
   * The `force_transfer` extrinsic transfers the blockchains native asset (PICA) from `origin` to `dest`
  ...
  /**
   * The `mint_into` extrinsic burms `amount` of `asset_id` of `dest` wallet.
   */
