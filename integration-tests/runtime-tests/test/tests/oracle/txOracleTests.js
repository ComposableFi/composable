...
const polkadotjs_1 = require("@composable/utils/polkadotjs"); // Weird import, because I had compilation problems.
/**
...
describe('tx.oracle Tests', function () {
    ...
    describe('tx.addAssetAndInfo Success Test', function () {
        ...
        it('Can add new asset and info', async function () {
            ...
            const { data: [result], } = await (0, addAssetAndInfoTests_1.txOracleAddAssetAndInfoSuccessTest)(controllerWallet, assetId, threshold, minAnswers, maxAnswers, blockInterval, reward, slash);
            if (result.isErr)
    ...
    describe('tx.setSigner Success Test', function () {
        ...
        it('Can set signer', async function () {
            ...
            const { data: [result], } = await (0, setSignerTests_1.runBeforeTxOracleSetSigner)(sudoKey, signedWallet); // Making sure we have funds.
            (0, chai_1.expect)(result.isOk).to.be.true;
            const { data: [resultAccount0, resultAccount1], } = await (0, setSignerTests_1.txOracleSetSignerSuccessTest)(controllerWallet, signedWallet)
    ...
    describe('tx.addStake Success Test', function () {
        ...
        it('Can add stake from creator/controller', async function () {
            if (!test_configuration_json_1.default.enabledTests.addStake__success.add1)
                this.skip();
            const sudoKey = walletAlice;
            await (0, addStakeTests_1.runBeforeTxOracleAddStake)(sudoKey, controllerWallet, signedWallet); // Preparing the signer to have funds.
            const stake = api.createType('u128', 250000000000);
            const { data: [result], } = await (0, addStakeTests_1.txOracleAddStakeSuccessTest)(controllerWallet, stake);
            (0, chai_1.expect)(result).to.not.be.an('Error');
            (0, chai_1.expect)(result.toString()).to.be
    ...
    describe('tx.submitPrice Success Test', function () {
        ...
        it('Can submit new price by signer', async function () {
            ...
            const assetId = api.createType('u128', newAsset1);
            const { data: [result], } = await (0, submitPriceTests_1.txOracleSubmitPriceSuccessTest)(signedWallet, price, assetId);
            (0, chai_1.expect)(result).to.not.be.an('Error');
            (0, chai_1.expect)(result.toString()).to.be
                .equal(api.createType('AccountId32', signedWallet.publicKey).toString());
        });
    ...
    describe('tx.removeStake Success Test', function () {
        ...
        it('Can remove stake', async function () {
            if (!test_configuration_json_1.default.enabledTests.removeStake__success.remove1)
                this.skip();
            const { data: [result], } = await (0, removeStakeTests_1.txOracleRemoveStakeSuccessTest)(controllerWallet);
            (0, chai_1.expect)(result).to.not.be.an('Error');
            (0, chai_1.expect)(result.toString()).to.be
                .equal(api.createType('AccountId32', signedWallet.publicKey).toString());
        });
    ...
    describe('tx.reclaimStake Success Test', function () {
        ...
        it('Can reclaim stake', async function () {
            ...
            await (0, polkadotjs_1.waitForBlocks)(unlockBlock.toNumber() - currentBlock.toNumber());
            const { data: [result], } = await (0, reclaimStakeTests_1.txOracleReclaimStakeSuccessTest)(controllerWallet);
            (0, chai_1.expect)(result).to.not.be.an('Error');
            (0, chai_1.expect)(result.toString()).to.be
                .equal(api.createType('AccountId32', signedWallet.publicKey).toString());
        });
