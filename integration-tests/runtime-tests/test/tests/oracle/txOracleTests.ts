...
describe("Oracle Tests", function () {
  ...
  describe("tx.submitPrice Success Test", function () {
    ...
    it("Can submit new price by signer", async function () {
      ...
      const assetId = api.createType("u128", newAsset1);
      Promise.all([
        txOracleSubmitPriceSuccessTest(api, controllerWallet, price, assetId),
        txOracleSubmitPriceSuccessTest(api, signerWallet1, price, assetId),
        txOracleSubmitPriceSuccessTest(api, signerWallet2, price, assetId),
