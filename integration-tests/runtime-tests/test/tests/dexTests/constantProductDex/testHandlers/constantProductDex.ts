...
//In order to successfully send funds, the users should have sufficient funds with assetId's 2 and 3 respetively.
export async function mintAssets(walletId:KeyringPair, assetId: number, amount: number){
  const {data: [result],} = await sendAndWaitForSuccess(
    api,
    walletAlice,
    api.events.sudo.Sudid.is,
    api.tx.sudo.sudo(
    api.tx.assets.mintInto(assetId, walletId.publicKey, amount)
    )
  );
  return result;    
}
