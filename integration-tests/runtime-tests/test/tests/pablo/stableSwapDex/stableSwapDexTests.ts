...
import {
  ...
  getPoolInfo, getUserTokens, removeLiquidityFromPool, rpcPriceFor, sellToPool, swapTokenPairs, transferTokens
...
describe("StableSwapDex Test Suite", function () {
  ...
  describe("StableSwapDex Create Pool Tests", async function () {
    ...
    it("Given that users have sufficient funds, User1 can create StableswapPool", async function () {
      const result = await createStableSwapPool(api, walletId1, walletId1, baseStableAssetId, quoteStableAssetId, ampCoefficient, fee, ownerFee);
  ...
  describe("StableSwapDex buy,sell and swap tests", async function () {
    ...
    it("Given that users have sufficient funds, User2 can buy from StableSwapPool and amounts are " +
      ...
      const rpcRes = await rpcPriceFor(api, api.createType("PalletPabloPoolId", poolId2),
        api.createType("CustomRpcCurrencyId", baseStableAssetId),
