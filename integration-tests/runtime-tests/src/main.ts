...
import {startBalanceGenerator} from "@composable/generators/setBalance/setStartBalance";
...
async function createDefaultData(api: ApiPromise, sudoKey: KeyringPair) {
  ...
  // Setting start balance for alice and bob accounts.
  await startBalanceGenerator.setBalance(api, sudoKey, walletAlice.address);
  await startBalanceGenerator.setBalance(api, sudoKey, walletBob.address);
}
