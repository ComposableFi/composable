...
contract Interpreter is IInterpreter {
    ...
    constructor(
        IGateway.Origin memory _origin,
    ...
    function _checkField(
        ...
        (success, newPos, _field, _type) = ProtobufLib.decode_key(
            pos,
            program
        );
    ...
    function _getMessageLength(bytes calldata program, uint64 pos)
        private
        returns (uint64 size, uint64 newPos)
    {
        // reading instruction message
        bool success;
        (success, newPos, size) = ProtobufLib.decode_embedded_message(
            pos,
            program
        );
        ...
        returns (address account, uint64 newPos)
    {
        ...
        pos = _checkField(
            program,
            1,
            ProtobufLib.WireType.LengthDelimited,
            pos
        );
        (size, pos) = _getMessageLength(program, pos);
    ...
    function _handleUint128(bytes calldata program, uint64 pos)
        internal
        returns (uint128 value, uint64 newPos)
    {
        bool success;
        uint64 size;
        ProtobufLib.WireType _type;
    ...
    function _handleAssetAmount(bytes calldata program, uint64 pos)
        internal
        returns (uint256 amount, uint64 newPos)
    {
        uint64 size;
        ...
        if (field == 2) {
            ...
            amount =
                (IERC20(asset).balanceOf(address(this)) * nominator) /
                denominator;
        } else if (field == 3) {
            // unit
    ...
    function _handleAssetId(bytes calldata program, uint64 pos)
        internal
        returns (address asset, uint64 newPos)
    {
        uint64 size;
        ...
        )
    {
        ...
        while (pos < totalAssetsLength) {
            (
                assetAddresses[count],
                amounts[count],
                pos
            ) = _handleAsset(program, pos);
    ...
    function _handleNetwork(bytes calldata program, uint64 pos)
        internal
    ...
    function _handleSalt(bytes calldata program, uint64 pos)
        internal
        ...
        returns (uint64 newPos)
    {
        ...
        address bridgeAddress = IGateway(gatewayAddress).getBridge(
            networkId,
            security
        );
        ...
        uint256[] memory amounts;
        if (pos < maxPos) {
            (newPos, assetAddresses, amounts) = _handleAssets(
                program,
                pos,
                bridgeAddress
            );
        ...
        returns (uint64 newPos)
    {
        ...
        if (pos < maxPos) {
            ...
            while (pos < totalBindingsLength) {
                uint64 position;
                bytes memory valueToReplace;
                (position, valueToReplace, pos) = _handleBinding(
                    program,
                    pos,
                    payload
                );
                payload = _replaceBytesByPosition(
                    payload,
