...
describe("Interpreter", function () {
  ...
  describe("interpreter with protobuf", function () {
                ...
                xcvm.createTransfer(
                  ...
                  [
                    xcvm.createAsset(
                      xcvm.createAssetId(1),
                      xcvm.createBalance(
                        xcvm.createRatio('1000000000000000000000000000', '2000000000000000000000000000')
                      )
                    )
                  ]
                )
    ...
    it("test call function using sdk", async function () {
      ...
      const payload = ethers.utils.concat([ethers.utils.arrayify(abiCoder.encode(['address'], [erc20.address])), ethers.utils.arrayify(functionSignature)])
      let xcvm = new XCVM();
      let programMessage =
        xcvm.createProgram(
          xcvm.createInstructions(
