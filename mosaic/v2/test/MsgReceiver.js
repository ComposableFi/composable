...
describe("Msg Receiver", async () => {
  ...
  beforeEach(async () => {
    ...
    await msgReceiverFactory.connect(deployer).setRelayer(relayerAddress);
    const msgReceiverAddress = await msgReceiverFactory.retrievePersona(userAddress);
    msgReceiver = await ethers.getContractAt("MsgReceiver", msgReceiverAddress);
  ...
  it("approve token and forward call", async () => {
    ...
    await expect(
      msgReceiver
        .connect(relayer)
        .approveERC20TokenAndForwardCall(
          100,
          erc20_token.address,
          feeAddress,
          erc20_token2.address,
