...
describe("MosaicVault - Test network", async () => {
  ...
  describe("Test IOU token", async () => {
    it("Should create IOU token when add whitelisted token", async () => {
      ...
      const iouTokenContract = await ethers.getContractAt("ReceiptBase", ERC20ContractIOUAddress);
    ...
    it("Should mint equal amount of IOU token when provide liquidity", async () => {
      ...
      //test if transfer event is emitted
      await expect(iouTokenContract.connect(tokensHolder).transfer(owner.address, allowance))
        .to.emit(iouTokenContract, "ReceiptTransferOperationExecuted")
        .withArgs(tokensHolder.address, owner.address, allowance);
    });
  ...
  async function getIOUTokenContract(whitelistedToken) {
    ...
    return ethers.getContractAt("ReceiptBase", iouTokenContract.underlyingIOUAddress);
  }
});
...
async function mockContracts() {
  ...
  const iouTokenFactory = await (await ethers.getContractFactory("TokenFactory"))
    .connect(owner)
    .deploy(MosaicVaultContract.address, vaultConfig.address, ERC20Contract.address);
