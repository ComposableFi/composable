{ self, ... }: {
  perSystem =
    ...
    let
      ...
      gov = "centauri10d07y265gmmuvt4z0w9aw880jnsr700j7g7ejq";
      name = "centaurid";
      ...
      ibc-lightclients-wasm-v1-msg-push-new-wasm-code = code: {
        ...
        "messages" = [
          {
            ...
            "from_address" = "${gov}";
            "to_address" = "centauri1cyyzpxplxdzkeea7kwsydadg87357qnamvg3y3";
            "amount" = [{ "denom" = "ppica"; "amount" = "10000000000"; }];
          }
        ];
        "metadata" = "AQ==";
        "deposit" = "100000000000ppica";
        ...
        #     "signer" = "centauri12smx2wdlyttvyzvzg54y2vnqwq2qjatescq89n";
        #     "code" = code;
        #   }
        # ];
        # "deposit" = "50000000000ppica";
        # "metadata" = "none";
        # "title" = "none";
        # "summary" = "none";
      };
      ...
      ics10-grandpa-cw-proposal =
        let
          code = builtins.readFile
            "${self'.packages.ics10-grandpa-cw}/lib/ics10_grandpa_cw.wasm.txt";
      ...
      centaurid-init = pkgs.writeShellApplication
        {
          ...
          text = ''
            ...
            #centaurid tx 08-wasm push-wasm ${self'.packages.ics10-grandpa-cw}/lib/ics10_grandpa_cw.wasm --from validator --keyring-backend test --gas 9021526220000 --fees 92000000166ppica --keyring-dir "$KEYRING_TEST" --chain-id "$CHAIN_ID" --yes
            sleep 10
      ...
      centaurid-gen = pkgs.writeShellApplication
        {
          ...
          text = ''
            ...
            function add-genesis-account () {
              centaurid --keyring-backend test add-genesis-account "$1" "1000000000000000000ppica" --keyring-backend test --home "$CENTAURI_DATA"          
            }
