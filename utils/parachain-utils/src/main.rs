...
async fn rotate_keys(port: Option<String>, key: String) -> Result<(), RpcError> {
    ...
    let author = http::connect::<AuthorClient<Hash, Hash>>(&uri)
        .await
        .unwrap();
    ...
    assert!(author
        .has_session_keys(keys.clone().encode().into())
        .await
        .unwrap());

    // now to set our session keys on cha cha cha
    let call = Call::Session(session::Call::set_keys {
        keys: keys.clone(),
        proof: vec![],
    });
    ...
    let account_index = chachacha_system_client
        .nonce(account_id.clone())
        .await
        .unwrap();
