...
async fn upgrade_runtime(code: Vec<u8>, state: &State<T>) -> Result<(), subxt::Error> {
	...
	let has_event = events.iter().any(|event| {
		matches!(event.event, ParachainSystem(parachain_system::Event::UpgradeAuthorized))
	});
	...
	let has_event = events.iter().any(|event| {
		matches!(event.event, ParachainSystem(parachain_system::Event::ValidationFunctionStored))
	});
	if !has_event {
		return Err(subxt::Error::Other("Failed to enactg upgrade".into()));
	}
