...
pub fn instantiate(
    ...
    msg.config.registry_address = deps
        .api
        .addr_validate(&msg.config.registry_address)?
        .into_string();
...
pub fn ibc_channel_open(
    ...
    match (msg.counterparty_version(), channel.order) {
        ...
        (Some(counter_version), _) if counter_version != XCVM_GATEWAY_IBC_VERSION => Err(
            ContractError::InvalidIbcVersion(counter_version.to_string()),
        ),
        ...
        _ => Ok(Some(Ibc3ChannelOpenResponse {
            version: XCVM_GATEWAY_IBC_VERSION.to_string(),
        })),
    }
}
