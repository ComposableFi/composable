...
pub fn instantiate(
    ...
    Ok(
        Response::new().add_event(Event::new(XCVM_INTERPRETER_EVENT_PREFIX).add_attribute(
...
pub fn handle_execute_step(
    ...
    while let Some((index, instruction)) = instruction_iter.next() {
        response = match instruction {
            ...
            instr => {
                return Err(ContractError::InstructionNotSupported(format!(
                    "{:?}",
                    instr
                )))
            }
...
pub fn interpret_call(
    ...
    let cosmos_msg: CosmosMsg = flat_cosmos_msg
        .try_into()
        .map_err(|_| ContractError::DataSerializationError)?;
    Ok(response
