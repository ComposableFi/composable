...
fn make_program<T: Network<EncodedCall = Vec<u8>>, U: Network<EncodedCall = Vec<u8>>>(
    ...
    Ok(
        ProgramBuilder::<T, CanonicalAddr, Funds>::new("PING".as_bytes().to_vec())
            .spawn::<_, U, (), _>(
                ...
                |child| {
                    Ok(child.call_raw(
                        serde_json::to_vec(&FlatCosmosMsg::Wasm(
                            FlatWasmMsg::<ExecuteMsg>::Execute {
                                contract_addr: String::from_utf8_lossy(&Vec::<u8>::from(
                                    remote_address,
                                ))
                                .to_string(),
                                msg,
