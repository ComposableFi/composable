...
pub async fn get_timed_out_packets_messages(
	...
	for (channel_id, port_id) in channel_whitelist {
		let source_channel_response = source.query_channel_end(source_height, channel_id, port_id.clone()).await?;
		let source_channel_end = ChannelEnd::try_from(source_channel_response.channel.ok_or_else(|| {
			Error::Custom(format!(
		...
		let connection_id = source_channel_end.connection_hops.get(0).ok_or_else(|| {
			Error::Custom("Channel end missing connection id".to_string())
		})?.clone();
